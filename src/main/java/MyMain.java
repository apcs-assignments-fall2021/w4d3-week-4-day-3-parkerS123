...
public class MyMain {
    ...
    public static void printEvensBackwards(int[] arr) {
        ...
        for (int i = arr.length - 1; i > 0; i--){ // how to go through a loop backwards
            if (arr[i] % 2 == 0){
    ...
    public static boolean containsDuplicates(int[] arr) {
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < arr.length; j++) { // compare each number with every other thing
                if (arr[i] == arr[j] && i != j){
                    return true;
                }
            }
    ...
    public static int[] bubbleSort(int[] arr) {
        for (int not_i = 0; not_i < arr.length; not_i++){
            for (int i = 1; i < arr.length; i++) {
                if (arr[i] < arr[i - 1]) {
                    int will_harris = arr[i];
                    arr[i] = arr[i - 1];
                    arr[i - 1] = will_harris;
